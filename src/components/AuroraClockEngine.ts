...
export class AuroraClockEngine {
  ...
  private createSereneAuroraSky(): void {
    const skyGeometry = new THREE.SphereGeometry(260, 48, 48);
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        // Night Base vs Warm Daytime Sky Background
        ...
        vec3 dayNadir    = vec3(0.045, 0.025, 0.04);
        ...
        // Gentle, slow-moving aurora curtains (calm, luminous celestial ribbons)
        float t = uTime * 0.045;
        float auroraMask = smoothstep(0.12, 0.65, dir.y);

        vec3 coord = vec3(dir.x * 1.6 + t * 0.2, dir.z * 1.6, dir.y * 1.2 + t * 0.08);
        float n1 = snoise(coord);
        float n2 = snoise(coord * 1.8 - vec3(t * 0.15, 0.0, t * 0.1));

        float ray = sin(dir.x * 12.0 + n1 * 2.5 + t * 0.5) * 0.5 + 0.5;
        ray = pow(ray, 3.2);

        float wave = sin(dir.z * 2.5 + dir.x * 1.8 + t * 0.4 + n2 * 1.8) * 0.5 + 0.5;
        wave = smoothstep(0.35, 0.85, wave);

        float auroraIntensity = (wave * 0.65 + ray * 0.35) * auroraMask * (0.6 + 0.4 * n1);

        // Palettes:
        // Night palette: emerald green + glacial cyan + subtle violet
        vec3 nightEmerald = vec3(0.05, 0.88, 0.48);
        vec3 nightCyan    = vec3(0.05, 0.72, 0.95);
        vec3 nightViolet  = vec3(0.65, 0.25, 0.85);
        vec3 nightAurora  = mix(nightEmerald, nightCyan, sin(dir.x * 3.0 + t) * 0.5 + 0.5);
        nightAurora       = mix(nightAurora, nightViolet, smoothstep(0.4, 0.8, dir.y) * 0.5);

        // Day warmth palette: warm golden-amber, rose peach, and celestial copper
        vec3 dayGold      = vec3(0.95, 0.68, 0.22);
        vec3 dayRose      = vec3(0.92, 0.38, 0.45);
        vec3 dayPeach     = vec3(0.85, 0.52, 0.75);
        vec3 dayAurora    = mix(dayGold, dayRose, sin(dir.x * 2.8 + t) * 0.5 + 0.5);
        dayAurora         = mix(dayAurora, dayPeach, smoothstep(0.35, 0.8, dir.y) * 0.6);

        vec3 auroraColor  = mix(nightAurora, dayAurora, uWarmth);

        vec3 finalColor = baseSky + auroraColor * auroraIntensity * 1.1;
  ...
  private createCosmicStars(): void {
    ...
    for (let i = 0; i < starCount; i++) {
      ...
      const radius = 200 + Math.random() * 25;
  ...
  private animate = (): void => {
    ...
    // 2. Update Aurora Sky Uniforms (slow, calm motion + day warmth)
    if (this.skyMaterial) {
